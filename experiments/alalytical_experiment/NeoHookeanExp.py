...
from model import BioConstitutiveModel, PsiModel
from torch_icnn_cann import FICNN, PICNNWapper
from utils import query_device

from analytical_strain_energy import (PK1_P, AnalyticalDataGenerator, GradET,
                                        GradPS, GradUT, NeoHookean, SampleF)


class NeoHookeanNNPsiModel(PsiModel):
    def __init__(self, num_material_params, num_dir):
        super(NeoHookeanNNPsiModel, self).__init__(num_material_params, num_dir)
        self.PsiIiso = PICNNWapper(x_dims=[3, 8, 8, 8, 1])
        self.PsiIaniso = PICNNWapper(x_dims=[2 * num_dir + 2 * num_dir**2, 2, 2, 1])

        self.PsiCom = PICNNWapper(x_dims=[3 + 2 * num_dir + 2 * num_dir**2, 4, 4, 4, 1])
        self.com_cof = torch.nn.Parameter(torch.tensor([1.0]))
    ...
    def forward(self, fibers_invariants, material_params=None):
        ...
        psi_com = self.PsiCom(fibers_invariants)  # * self.com_cof

        return self.Psi(torch.cat((psi_iso, psi_aniso, psi_com), dim=1))


def train(epochs=30000, n_samples=2500):
    # logger.add("logs/neo_hookean_{time}.log")
    device = query_device()

    F, P = AnalyticalDataGenerator(
        sampler=SampleF,
        N_samples=n_samples,
        energy_fn=NeoHookean(c1=1.0 / math.sqrt(2), c2=10 / 3.0),
    )
    ...
    model.PsiSubNet = NeoHookeanNNPsiModel(model.num_material_params, model.num_dir)
    ...
    for i in range(epochs):
        ...
        if (i + 1) % 1000 == 0:
            ...
            logger.info(f"  it: {i+1} loss: {loss}")
        optimizer.step()

    # model.writer.close()
    torch.save(model, "outputs/neohookean/analytical_neo_hookean.pth")


def predict():
    model = torch.load(
        "outputs/neohookean/analytical_neo_hookean.pth", map_location=torch.device("cpu")
    )
    device = query_device()
    model.to(device)

    neo_hookean = NeoHookean(c1=1.0 / math.sqrt(2), c2=10.0 / 3.0)
    ...
    psi_ut = neo_hookean(F_UT)
    P_UT = PK1_P(psi_ut, F_UT).to(device)
    psi_et = neo_hookean(F_ET)
    P_ET = PK1_P(psi_et, F_ET).to(device)
    psi_ps = neo_hookean(F_PS)
    P_PS = PK1_P(psi_ps, F_PS).to(device)

    # 预测
    W_pred, P11_pr, P_UT_pred, S_pred, sigma_pred, _ = model(F_UT)
    W_pred, P11_pr, P_ET_pred, S_pred, sigma_pred, _ = model(F_ET)
    W_pred, P11_pr, P_PS_pred, S_pred, sigma_pred, _ = model(F_PS)

    def plotstresses(x_gt, y_gt, x_pr, y_pr):
        fig, ax = plt.subplots(3, 3, figsize=(12, 8))
        labels = [["UT", "ET", "PS"], ["UT", "ET", "PS"], ["UT", "ET", "PS"]]
        Plabels = [11, 22, 33]
        for axi, x_gti, y_gti, x_pri, y_pri, label, Plabel in zip(
            ax, x_gt, y_gt, x_pr, y_pr, labels, Plabels
        ):
            for axii, x_gtii, y_gtii, x_prii, y_prii, labeli in zip(
                axi, x_gti, y_gti, x_pri, y_pri, label
            ):
    ...
    fig, ax = plotstresses(
        [
            [lam_ut, lam_ut, lam_ut],
            [lam_ut, lam_ut, lam_ut],
            [lam_ut, lam_ut, lam_ut],
        ],  # [[1,2,3],[4,5,6]],#
        [
            [P_UT_11, P_ET_11, P_PS_11],
            [P_UT_22, P_ET_22, P_PS_22],
            [P_UT_33, P_ET_33, P_PS_33],
        ],
        [[lam_ut, lam_ut, lam_ut], [lam_ut, lam_ut, lam_ut], [lam_ut, lam_ut, lam_ut]],
        [
            [P_UT_11_pred, P_ET_11_pred, P_PS_11_pred],
            [P_UT_22_pred, P_ET_22_pred, P_PS_22_pred],
            [P_UT_33_pred, P_ET_33_pred, P_PS_33_pred],
        ],
    )

    plt.tight_layout()
    plt.savefig("outputs/neohookean/analytical_neo_hookean.eps", dpi=600)
    # plt.show()
