...
from utils import TrainTimer, query_device
...
def train(epochs, n_samples):
    ...
    logger.add("../logs/bonet_{time}.log")
    device = query_device()

    # F, P = AnalyticalDataGenerator(sampler=SampleF, N_samples=n_samples, energy_fn=BonetHyperelasticity())
    F, P = AnalyticalDataGenerator(
        sampler=SampleF2, N_samples=n_samples, energy_fn=BonetHyperelasticity()
    )
    ...
    torch.save(model, "../outputs/BonetHyperelasticity.pth")


def predict():
    model = torch.load(
        "../outputs/BonetHyperelasticity.pth", map_location=torch.device("cpu")
    )
    device = query_device()
    model.to(device)

    bonet_hyper = BonetHyperelasticity()

    # 生成测试数据
    lam_ut = np.linspace(0.8, 1.2, 50)
    ...
    plt.tight_layout()
    # plt.savefig('../outputs/figures_bonet.eps', dpi=600)
    plt.show()
