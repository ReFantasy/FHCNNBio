    ...
    for elem in root.findall('Boundary'):  # 遍历根节点的所有子节点
        for sub_elem in elem.findall('bc'):
            ...
            if sub_elem.get('name') == 'PrescribedDisplacement2':
                value = sub_elem.find('value')
                value.text = str(-lam)
            if sub_elem.get('name') == 'PrescribedDisplacement3':
                value = sub_elem.find('value')
                value.text = str(lam)
            if sub_elem.get('name') == 'PrescribedDisplacement4':
                value = sub_elem.find('value')
                value.text = str(-lam)
...
if __name__ == "__main__":
    # 定义拉伸量
    lams = np.linspace(-0.015, 0.015, 100)
    Fs = []
    sigmas = []

    tree = ET.parse('equibiaxial_xy.feb')  # 解析XML文件
    root = tree.getroot()
